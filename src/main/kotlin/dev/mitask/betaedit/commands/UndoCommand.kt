...
class UndoCommand : CommandProvider {
    override fun get(): LiteralArgumentBuilder<GlassCommandSource> {
        return literal("/undo")
            .requires {
                ...
                return@requires booleanPermission("betaedit.undo").test(it)
            }
            .executes { undo(it) }
    }

    fun undo(context: CommandContext<GlassCommandSource>): Int {
        ...
        val time = measureTimeMillis {
            history.blocks.forEach {
                world.setBlock(it.pos.x, it.pos.y, it.pos.z, it.blockId, it.meta)
            }
        }
        ...
        sendFeedbackAndLog(
            context.getSource(),
            "§aUndid ${cuboid.volume} blocks in ${if(time > 5000) "${time / 1000} seconds" else "$time ms"}"
        )
